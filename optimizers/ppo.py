...
class PPOOptimizer:
    def __init__(self, state_dim, action_dim, config, device):
        ...
        self.optimizer = torch.optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': config.PPO_LR_ACTOR},
            {'params': self.policy.critic.parameters(), 'lr': config.PPO_LR_CRITIC}
        ])
    ...
    def run(self, evaluate_weights_fn, state_vector, epochs, **kwargs):
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch+1}/{epochs} [PPO] | Weights: {np.round(weights,2)} -> Acc: {current_acc:.2f}% | Reward: {reward:.4f}")
