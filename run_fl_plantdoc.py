# run_fl_plantdoc.py - 专门为PlantDoc小样本数据集优化的联邦学习训练脚本
# 基于run_fl_optimized.py的逻辑，但针对小样本数据集进行了优化
...
# 导入我们自己的模块
import config_plantdoc_optimized as config  # 使用专门为PlantDoc优化的配置
from src.data_loader_plantdoc import get_farm_dataloaders_plantdoc  # 使用专门的数据加载器
...
def save_teacher_model(teacher_model, farm_id, round_idx, output_dir):
    """
    保存教师模型状态
    
    Args:
        teacher_model: 要保存的教师模型
        farm_id: 农场ID
        round_idx: 当前轮次
        output_dir: 输出目录
    """
    teacher_dir = os.path.join(output_dir, 'teacher_models')
    os.makedirs(teacher_dir, exist_ok=True)
    
    save_path = os.path.join(teacher_dir, f'teacher_{farm_id}_round_{round_idx}.pth')
    torch.save({
    ...
    print(f"      教师模型已保存: {save_path}")

def main():
    """
    主执行函数，专门为PlantDoc小样本数据集优化
    """
    print("--- PlantDoc小样本数据集优化版：层级冻结FL + 蒸馏 + 量化框架 ---")
    print(f"使用设备: {config.DEVICE}")
    print_memory_usage("程序启动时")

    # 1. 初始化
    all_farms_data_loaders, global_val_loader = get_farm_dataloaders_plantdoc(
    ...
    for server_round_idx in range(config.SERVER_ROUNDS):
        ...
        for farm_id in active_farm_ids:
            ...
            for farm_fl_round in range(config.FARM_FL_ROUNDS):
                ...
                print(f"    内部FL轮次 {farm_fl_round + 1}/{config.FARM_FL_ROUNDS} | 学生模型 ({config.DISTILL_MODEL_ARCH}):")
                print_metrics(student_metrics, "      ")
                print(f"    内部FL轮次 {farm_fl_round + 1}/{config.FARM_FL_ROUNDS} | 教师模型 ({config.MODEL_ARCHITECTURE}):")
            ...
            # --- 新增：保存教师模型 ---
            # 保存第一个客户端的教师模型作为代表
            representative_teacher = local_teachers[farm_id][0]
            save_teacher_model(representative_teacher, farm_id, server_round_idx + 1, config.OUTPUT_DIR)
